...
func TestPlayoutDirect(t *testing.T) {
	t.Run("direct-list-playout", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"station01"} {
...
func playoutDirectSetup(mockres any) *playoutDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"ENERGY_RADIO_STATIONS_TEST_PLAYOUT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ENERGY_RADIO_STATIONS_TEST_PLAYOUT_ENTID"]; ok {
