...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ENERGYRADIOSTATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("ENERGYRADIOSTATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ENERGYRADIOSTATIONS_TEST_EXPLAIN"); explain != "" {
