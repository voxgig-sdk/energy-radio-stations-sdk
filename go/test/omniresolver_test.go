...
import (
	...
	omni "github.com/voxgig-sdk/energy-radio-stations-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EnergyRadioStationsError); is {
				out := map[string]any{
					"name":    "EnergyRadioStationsError",
...
func omniCtx(arg any, client *sdk.EnergyRadioStationsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EnergyRadioStationsSDK); is {
			client = live
