...
// argument it returns the current data as an Playout; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *PlayoutEntity) DataTyped(data ...Playout) Playout {
	if len(data) > 0 {
		return typedFrom[Playout](e.Data(asMap(data[0])))
	}
	return typedFrom[Playout](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Playout (all fields
// optional at the wire level).
func (e *PlayoutEntity) MatchTyped(match ...Playout) Playout {
	if len(match) > 0 {
		return typedFrom[Playout](e.Match(asMap(match[0])))
	}
	return typedFrom[Playout](e.Match())
}
...
func (e *PlayoutEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
// PlayoutListMatch and returns []Playout. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *PlayoutEntity) ListTyped(reqmatch PlayoutListMatch, ctrl map[string]any) ([]Playout, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Playout](res), nil
}
