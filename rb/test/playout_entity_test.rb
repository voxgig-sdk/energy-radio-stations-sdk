...
class PlayoutEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = EnergyRadioStationsSDK.test(seed, nil)
    seen = base.Playout(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Playout(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ENERGY_RADIO_STATIONS_TEST_PLAYOUT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def playout_basic_setup(extra)
  ...
  entid_env_raw = ENV["ENERGY_RADIO_STATIONS_TEST_PLAYOUT_ENTID"]
  ...
  if env["ENERGY_RADIO_STATIONS_TEST_LIVE"] == "TRUE"
    ...
    client = EnergyRadioStationsSDK.new(Helpers.to_map(merged_opts))
  end
