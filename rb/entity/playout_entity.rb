...
class PlayoutEntity
  ...
  def data_set(args)
    if args
      @_data = EnergyRadioStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Playout, Hash] the current Playout data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EnergyRadioStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Playout fields)
  def match_get
  ...
  #
  # @param reqmatch [PlayoutListMatch, Hash, nil] match filter (any subset of
  #   Playout fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Playout>, Array] the matching Playout items; raises EnergyRadioStationsError on failure
