...
boru-driven command-line client **and** interactive REPL for the EnergyRadioStations
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/energy-radio-stations-cli)
make build

# 2. See usage (words, entities, env vars)
./energy-radio-stations-cli --help
...
ENERGY_RADIO_STATIONS_BASE=https://api.example.com ./energy-radio-stations-cli list playout

# 6. No arguments -> interactive REPL
./energy-radio-stations-cli
energy-radio-stations> list playout
energy-radio-stations> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/energy-radio-stations-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/energy-radio-stations-cli list playout
   ```
...
Run with no arguments to open a REPL (prompt `energy-radio-stations>`). Each line is
evaluated as its own boru expression:

```text
$ ./energy-radio-stations-cli
energy-radio-stations> list playout
energy-radio-stations> /help
energy-radio-stations> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `playout`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/energy-radio-stations-cli`. |
...
(`./energy-radio-stations-cli <expr>`) and interactively (the REPL), and expressions compose the
