...
class TestPlayoutEntity:
    ...
    def test_should_stream(self):
        ...
        base = EnergyRadioStationsSDK.test(seed, None)
        seen = list(base.Playout(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = EnergyRadioStationsSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Playout(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ENERGY_RADIO_STATIONS_TEST_PLAYOUT_ENTID JSON to run live")
        client = setup["client"]
...
def _playout_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ENERGY_RADIO_STATIONS_TEST_PLAYOUT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("ENERGY_RADIO_STATIONS_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = EnergyRadioStationsSDK(helpers.to_map(merged_opts))

    _live = env.get("ENERGY_RADIO_STATIONS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ENERGY_RADIO_STATIONS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
