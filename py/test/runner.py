# EnergyRadioStations SDK test runner
...
class EnergyRadioStationsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EnergyRadioStationsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EnergyRadioStationsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EnergyRadioStationsTestRunner.getenv("ENERGY_RADIO_STATIONS_TEST_LIVE")
        override = EnergyRadioStationsTestRunner.getenv("ENERGY_RADIO_STATIONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EnergyRadioStationsTestRunner.getenv(key)
        ...
        explain = EnergyRadioStationsTestRunner.getenv("ENERGY_RADIO_STATIONS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ENERGY_RADIO_STATIONS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EnergyRadioStationsTestRunner._test_control is not None:
            return EnergyRadioStationsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EnergyRadioStationsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EnergyRadioStationsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EnergyRadioStationsTestRunner.load_test_control()
...
def load_env_local():
    EnergyRadioStationsTestRunner.load_env_local()


def env_override(m):
    return EnergyRadioStationsTestRunner.env_override(m)


def entity_data(v):
    return EnergyRadioStationsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EnergyRadioStationsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EnergyRadioStationsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EnergyRadioStationsTestRunner.load_test_control()


def live_client_options():
    return EnergyRadioStationsTestRunner.live_client_options()


def live_delay_ms():
    return EnergyRadioStationsTestRunner.live_delay_ms()
