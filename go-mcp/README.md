# energy-radio-stations-mcp

[MCP](https://modelcontextprotocol.io) server exposing the EnergyRadioStations SDK as
two agent tools — `energy-radio-stations_list` and `energy-radio-stations_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/energy-radio-stations-mcp)
make build

# 2. Provide credentials via the environment
export ENERGY_RADIO_STATIONS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user energy-radio-stations \
  -- /absolute/path/to/energy-radio-stations-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./energy-radio-stations-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// energy-radio-stations_list: first page of records
{ "entity": "playout" }
{ "entity": "playout", "query": { } }

// energy-radio-stations_load: one record by id
{ "entity": "playout", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/energy-radio-stations-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user energy-radio-stations \
     -- "$PWD"/dist/*/energy-radio-stations-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `energy-radio-stations_list` and `energy-radio-stations_load` tools now appear
   in new sessions. Ask the agent to *"list playout using energy-radio-stations"*
   and it calls `energy-radio-stations_list` with `{"entity":"playout"}`.
...
```sh
export ENERGY_RADIO_STATIONS_APIKEY=sk_live_xxx            # API key
export ENERGY_RADIO_STATIONS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `energy-radio-stations_list` tool
...
### Call the `energy-radio-stations_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `energy-radio-stations_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `energy-radio-stations_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ENERGY_RADIO_STATIONS_APIKEY` | API key sent with every request. |
| `ENERGY_RADIO_STATIONS_BASE` | Optional override of the API base URL. |
...
```sh
./energy-radio-stations-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"energy-radio-stations_load","arguments":{"entity":"playout","query":{"id":1}}}}'
```
