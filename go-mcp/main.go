// Command EnergyRadioStations-mcp is an MCP server exposing the EnergyRadioStations
// SDK as MCP tools.
//
// Two tools are registered:
//
//	EnergyRadioStations_list  — list records of an entity
//	EnergyRadioStations_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: ENERGY_RADIO_STATIONS_APIKEY carries the API key and
	// ENERGY_RADIO_STATIONS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ENERGY_RADIO_STATIONS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ENERGY_RADIO_STATIONS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEnergyRadioStationsSDK(opts)
