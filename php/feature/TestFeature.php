...
// EnergyRadioStations SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class EnergyRadioStationsTestFeature extends EnergyRadioStationsBaseFeature
{
    ...
    public function init(EnergyRadioStationsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (EnergyRadioStationsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = EnergyRadioStationsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (EnergyRadioStationsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(EnergyRadioStationsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = EnergyRadioStationsParam::call($ctx, $k);
