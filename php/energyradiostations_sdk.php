...
class EnergyRadioStationsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = EnergyRadioStationsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = EnergyRadioStationsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!EnergyRadioStationsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, EnergyRadioStationsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return EnergyRadioStationsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = EnergyRadioStationsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = EnergyRadioStationsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = EnergyRadioStationsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new EnergyRadioStationsError($op . "_allow",
                "EnergyRadioStationsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = EnergyRadioStationsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new EnergyRadioStationsError("graphql_error",
                "EnergyRadioStationsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_playout = null;

    // Canonical facade: $client->Playout()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->playout()
    // resolves here too.
    public function Playout($data = null)
    {
        require_once __DIR__ . '/entity/playout_entity.php';
        if ($data === null) {
            if ($this->_playout === null) {
                $this->_playout = new PlayoutEntity($this, null);
            }
            return $this->_playout;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new EnergyRadioStationsSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
