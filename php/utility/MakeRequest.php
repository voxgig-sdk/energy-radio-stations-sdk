...
class EnergyRadioStationsMakeRequest
{
    public static function call(EnergyRadioStationsContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new EnergyRadioStationsResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new EnergyRadioStationsResponse($fetched);
        } else {
