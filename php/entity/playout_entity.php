...
class PlayoutEntity
{
    ...
    /**
     * @param Playout|array $args Playout data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EnergyRadioStationsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Playout|array The current Playout data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Playout fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EnergyRadioStationsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Playout fields).
     */
    ...
    /**
     * List Playout items matching the given filter.
     *
     * @param PlayoutListMatch|array|null $reqmatch Match filter (any subset
     *   of Playout fields) as an assoc-array; PlayoutListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Playout[]|array A list of Playout items as assoc-arrays at
     *   the SDK boundary; throws EnergyRadioStationsError on failure (item-5 convention).
     */
