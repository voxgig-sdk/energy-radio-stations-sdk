...
//     (\`new EnergyRadioStationsSDK(...)\` / \`EnergyRadioStationsSDK.test(...)\` become
//     \`EnergyRadioStationsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'EnergyRadioStationsSDK'
...
const TEST_SEED = {"entity":{"playout":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EnergyRadioStationsSDK(...)\` and any
// \`EnergyRadioStationsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EnergyRadioStationsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EnergyRadioStationsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
