...
module EnergyRadioStationsUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = EnergyRadioStationsContext.new({}, nil)
    end
    op = ctx.op || EnergyRadioStationsOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || EnergyRadioStationsResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(EnergyRadioStationsError) ? err.msg : err.to_s
    ...
    sdk_err = EnergyRadioStationsError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(EnergyRadioStationsError)
