...
module EnergyRadioStationsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EnergyRadioStationsResponse.new({})
    ...
    if fetch_err
      ...
      response = EnergyRadioStationsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EnergyRadioStationsResponse.new(fetched)
    else
