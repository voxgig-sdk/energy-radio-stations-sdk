...
// EnergyRadioStations SDK retry feature
...
class EnergyRadioStationsRetryFeature extends EnergyRadioStationsBaseFeature
{
    ...
    public function init(EnergyRadioStationsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EnergyRadioStationsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EnergyRadioStationsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EnergyRadioStationsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
