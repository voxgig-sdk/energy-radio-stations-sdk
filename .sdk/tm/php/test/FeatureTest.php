...
// EnergyRadioStations SDK feature test
//
...
class FtCtrl extends EnergyRadioStationsControl
{
...
class FtClient
{
    ...
    private EnergyRadioStationsUtility $utility;

    public function __construct(EnergyRadioStationsUtility $utility, array $options)
    {
    ...
    public function get_utility(): EnergyRadioStationsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EnergyRadioStationsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EnergyRadioStationsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EnergyRadioStationsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EnergyRadioStationsConfig::make_feature($name);
    ...
    private static function buildUrl(EnergyRadioStationsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EnergyRadioStationsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EnergyRadioStationsResponse($fetched);
        }
    ...
    private function fail(EnergyRadioStationsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EnergyRadioStationsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
