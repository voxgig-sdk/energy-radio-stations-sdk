...
// EnergyRadioStations SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = EnergyRadioStationsOmni::makeStructRunner(
    ...
    {
        return EnergyRadioStationsOmni::objmodel(EnergyRadioStationsOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return EnergyRadioStationsOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return EnergyRadioStationsOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(EnergyRadioStationsOmni::structfix($now, Struct::undef()))
                        !== json_encode(EnergyRadioStationsOmni::structfix($val, Struct::undef()))) {
