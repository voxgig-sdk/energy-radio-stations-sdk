...
//    only - a typed EnergyRadioStationsContext there would make every ctx
...
final class EnergyRadioStationsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new EnergyRadioStationsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new EnergyRadioStationsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new EnergyRadioStationsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new EnergyRadioStationsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new EnergyRadioStationsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?EnergyRadioStationsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new EnergyRadioStationsError(is_string($code) ? $code : '', $msg);
    }
